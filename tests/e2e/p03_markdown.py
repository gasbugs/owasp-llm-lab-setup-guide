"""Extract P03 solution source without executing Markdown shell commands."""
import ast
import re
...
def extract_solution(document):
    ...
    blocks = re.findall(
        r"^cat > llm-security-control-plane/guided-labs/h03-ingestion-search/learner.py <<'EOF'\n(.*?)^EOF$",
    ...
    functions = [node for node in tree.body if isinstance(node, ast.AsyncFunctionDef)
                 and node.name == 'search_current']
    if len(functions) != 1:
        raise ValueError('one async search_current function is required')
    return source
