"""Extract the P12 solution without executing document shell commands."""
import ast
import re
...
def extract_solution(document):
    ...
    blocks = re.findall(
        r"^cat > llm-security-control-plane/guided-labs/h12-application-pipeline/pipeline.py <<'EOF'\n(.*?)^EOF$",
        parts[1], re.MULTILINE | re.DOTALL)
    if len(blocks) != 1:
        raise ValueError('one complete pipeline.py heredoc is required')
    source = blocks[0]
    ...
    if len(functions) != 1:
        raise ValueError('one async handle_request function is required')
    return source
