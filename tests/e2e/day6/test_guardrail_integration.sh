...
docker run --rm --network none -e GUARD_MODE=enforce \
  -v "$ROOT/tests/e2e/day6/check_fail_closed.py:/tmp/check_fail_closed.py:ro" \
...
docker rm -f day6-presidio-api >/dev/null
start_presidio enforce true
...
docker rm -f day6-guardrail-ui >/dev/null
...
! docker logs day6-guardrail-ui 2>&1 | grep -F 'customer.demo@example.com'
! docker logs llm-security-nemo-dialog-rails 2>&1 | grep -F 'customer.demo@example.com'

printf 'LOGS\n'
docker logs day6-presidio-api | tee "$WORK/presidio-api.log"
! grep -F 'alice@example.com' "$WORK/presidio-api.log"
! grep -F '4111 1111 1111 1111' "$WORK/presidio-api.log"
docker logs llm-security-nemo-dialog-rails
docker logs day6-guardrail-ui
