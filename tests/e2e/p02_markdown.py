"""Extract P02's complete solution; never execute document shell commands."""
import ast
import re
...
def extract_solution(document):
    ...
    blocks = re.findall(
        r"^cat > llm-security-control-plane/guided-labs/h02-document-ingestion/learner.py <<'EOF'\n(.*?)^EOF$",
    ...
    functions = [node for node in tree.body if isinstance(node, ast.AsyncFunctionDef)
                 and node.name == 'prepare_document']
    if len(functions) != 1:
        raise ValueError('one async prepare_document function is required')
    return source
