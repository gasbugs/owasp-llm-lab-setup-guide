...
class LiveControllerContractTest(unittest.TestCase):
    ...
    def test_cost_and_cleanup_contracts_are_fail_closed(self) -> None:
        source = read("run-commit-live-validation.sh")
        self.assertIn("RUN_DEADLINE_MINUTES:=120", source)
        self.assertNotIn('"-var=student_id=', source)
        self.assertNotIn('"-var=student_ids=', source)
        self.assertNotIn('"-var=course_dates=', source)
        self.assertNotIn('"-var=course_start_date=', source)
        self.assertNotIn('"-var=course_budget_usd=', source)
        self.assertNotIn('"-var=monthly_budget_usd=', source)
        self.assertNotIn('"-var=daily_budget_usd=', source)
        self.assertNotIn("TF_VAR_alert_email", source)
        self.assertNotIn('"-var=enable_auto_stop=', source)
        self.assertNotIn('"-var=auto_stop_schedule_mode=', source)
        self.assertNotIn("auto_stop_schedule", source)
