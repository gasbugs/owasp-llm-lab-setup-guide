...
class PrepareModule08ContractTests(unittest.TestCase):
    ...
    def test_script_verifies_behavior_and_content_safety(self) -> None:
        ...
        self.assertIn('service_name="llm-security-gateway"', self.source)
        self.assertIn('.event_type == "application_authentication"', self.source)
        self.assertIn('.attributes.guard_engine == "nemo"', self.source)
        self.assertIn('.attributes.guard_engine == "presidio"', self.source)
