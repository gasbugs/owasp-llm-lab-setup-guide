"""H07 one-time role capability and Gateway ledger contract tests."""
...
SOURCE = (
    ROOT
    / "llm-security-control-plane/guided-bedrock-gateway/h07_backend.py"
)
...
class FakeBedrockRuntime:
    ...
    def converse(self, **kwargs):
        ...
        return {
            "ResponseMetadata": {"RequestId": "aws-h07-request-1"},
...
class GuidedH07GatewayTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        spec = importlib.util.spec_from_file_location(
            "guided_h07_gateway_tests", SOURCE
        )
    ...
    def create_suite(self, suite_id: str) -> dict:
        response = self.client.post(
            ...
            headers={"Authorization": "Bearer h07-control"},
        )
    ...
    def get_ledger(self, suite_id: str) -> dict:
        response = self.client.get(
            f"/v1/h07/suites/{suite_id}/ledger",
            headers={"Authorization": "Bearer h07-verifier"},
        )
        self.assertEqual(response.status_code, 200, response.text)
        return response.json()

    def test_suite_is_server_owned_and_capabilities_are_role_bound(self):
        suite_id = "07000000-0000-0000-0000-000000000001"
        ...
        self.assertEqual(set(normal["roles"]), {"content_safety", "main"})

        guard = normal["roles"]["content_safety"]
    ...
    def test_contract_guard_results_and_main_metadata_are_separate(self):
        suite_id = "07000000-0000-0000-0000-000000000002"
        suite = self.create_suite(suite_id)
        normal, risk = suite["cases"]
        normal_guard = self.invoke(normal["roles"]["content_safety"])
        risk_guard = self.invoke(risk["roles"]["content_safety"])
        main = self.invoke(normal["roles"]["main"])
        self.assertEqual(json.loads(normal_guard.json()["choices"][0]["message"]["content"])["User Safety"], "safe")
        self.assertEqual(json.loads(risk_guard.json()["choices"][0]["message"]["content"])["User Safety"], "unsafe")
        self.assertEqual(main.status_code, 200)

        ledger = self.get_ledger(suite_id)
        guard_calls = [item for item in ledger["calls"] if item["role"] == "content_safety"]
        main_call = next(item for item in ledger["calls"] if item["role"] == "main")
        self.assertEqual([item["safety_result"] for item in guard_calls], ["safe", "unsafe"])
        ...
        self.assertEqual(main_call["safety_result"], "not_applicable")
    ...
    def test_invalid_guard_json_is_recorded_without_becoming_safe(self):
        suite_id = "07000000-0000-0000-0000-000000000003"
        suite = self.create_suite(suite_id)
        grant = suite["cases"][0]["roles"]["content_safety"]
    ...
    def test_expired_capability_is_rejected_and_recorded(self):
        suite_id = "07000000-0000-0000-0000-000000000008"
        ...
        self.assertEqual(expired.json()["detail"], "H07 capability expired")
    ...
    def test_ledger_is_verifier_only_and_suite_input_is_strict(self):
        suite_id = "07000000-0000-0000-0000-000000000005"
        rejected = self.client.post(
            "/v1/h07/suites",
            json={
                "suite_id": suite_id,
    ...
    def test_aws_mode_uses_fixed_nova_model_and_persists_native_evidence(self):
        suite_id = "07000000-0000-0000-0000-000000000006"
        ...
        call = self.get_ledger(suite_id)["calls"][0]
        self.assertEqual(call["provider_request_id"], "aws-h07-request-1")
    ...
    def test_aws_failure_leaves_capability_failed(self):
        suite_id = "07000000-0000-0000-0000-000000000007"
        with patch.object(self.backend, "PROVIDER_MODE", "aws"):
            suite = self.create_suite(suite_id)
            grant = suite["cases"][0]["roles"]["main"]
            error = ClientError(
