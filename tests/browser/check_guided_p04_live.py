"""P04 real Browser contract request; raw response, verdict, help, themes, sizes."""
import argparse
import json
...
def main():
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            assert 'invoke_guarded' in page.locator('#h04-work').inner_text()
            if args.prepare_aws:
                with page.expect_response(lambda r: r.url.endswith('/api/practice/P04/provision'), timeout=510000) as pending_preparation:
                    page.locator('#h04-provision').click()
                preparation_response = pending_preparation.value
                preparation = preparation_response.json()
                args.output.with_suffix('.preparation.json').write_text(json.dumps(preparation, ensure_ascii=False, indent=2))
                ...
                page.wait_for_function('document.querySelector("#task-completion").textContent === "P04 과제: 미완료"')
                assert json.loads(page.locator('#raw').inner_text()) == preparation
            with page.expect_response(lambda r: r.url.endswith('/api/practice/P04/verify'), timeout=240000) as pending:
            ...
            assert response.status == 200 and result['activity_id'] == 'P04'
            assert result['task_completed'] is complete
            assert result['security_verdict'] == ('PASS' if complete else 'ERR')
            page.wait_for_function('(label) => document.querySelector("#task-completion").textContent === label',
                                   arg='P04 과제: ' + ('완료' if complete else '미완료'))
            ...
            assert page.locator('#requested-max').inner_text() == ('27' if complete else '—')
            assert page.locator('#forwarded-max').inner_text() == ('4' if complete else '—')
            if complete:
            ...
            rejected = page.evaluate('''async () => {
                ...
                return (await fetch('/api/practice/P04/verify', {method: 'POST',
            ...
            for width in (1440, 390):
                ...
                for action in ('h04-provision', 'h04-verify'):
