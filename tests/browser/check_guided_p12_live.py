"""P12 through the actual UI/proxy/services; no response fixtures."""
import argparse
import json
...
def main():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            with page.expect_response(lambda response: response.url.endswith('/api/practice/P12/verify'), timeout=990000) as pending:
            ...
            label = 'P12 과제: ' + ('완료' if expected else '미완료')
            ...
            assert page.locator('#provider-label').inner_text() == '검사 묶음'
            assert page.locator('#provider-id').inner_text() == result['execution_id']
            assert page.locator('#requested-max').inner_text() == ('8' if expected else '0')
            assert page.locator('#forwarded-max').inner_text() == ('16' if expected else '—')
            boundary = page.evaluate('''async () => {
                const bootstrap = await (await fetch('/api/bootstrap')).json();
                return (await fetch('/api/practice/P12/verify', {method: 'POST',
            ...
            for width in (1440, 390):
                ...
                assert page.evaluate('''() => {
                    const tip = document.querySelector('#h12-verify-help').getBoundingClientRect();
                ...
                help_button = page.locator('[aria-describedby="h12-verify-help"]')
                help_button.focus()
                ...
                page.locator('#h12-work').scroll_into_view_if_needed()
            ...
            args.output.write_text(json.dumps({'scope': 'Browser→proxy→Control Center→P12→native products→TCP verifier; provider mode recorded by publisher',
