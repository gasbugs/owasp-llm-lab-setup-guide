"""Trusted P04 lifecycle. Records execution but never decides course completion."""
import hashlib
import json
import math
import re
...
class LifecycleError(RuntimeError):
    def __init__(self, status=None):
        super().__init__("P04 lifecycle request failed")
        self.status = status
...
class Workflow:
    ...
    def _post(self, path, body, *, timeout=5):
        try:
            with httpx.Client(timeout=timeout, follow_redirects=False, trust_env=False, transport=self.transport) as client:
                with client.stream("POST", self.origin + "/v1/p04" + path, json=body,
    ...
    def prepare_suite(self, *, suite_id, executions, timeout=30):
        ...
        for row in executions:
            if (not isinstance(row, dict) or set(row) != {"case_id", "execution_id"} or str(UUID(row["execution_id"])) != row["execution_id"]
        ...
        mapping_digest = hashlib.sha256(json.dumps(executions, sort_keys=True, ensure_ascii=False, allow_nan=False).encode()).hexdigest()
        response = self._post("/suites", {"suite_id": suite_id, "cases": executions}, timeout=timeout)
        if response.get("prepared") is not True or response != {"suite_id": suite_id, "contract_version": "p04-guardrail-v1",
    ...
    def run_case(self, source_path, *, suite_id, execution_id, runner_digest, timeout=75):
        ...
        if (not isinstance(runner_digest, str) or not re.fullmatch(r"[0-9a-f]{64}", runner_digest)
                or type(timeout) not in (int, float) or not 0 < timeout <= 75):
        ...
        try:
            ...
            if (grant.get("suite_id") != suite_id or grant.get("execution_id") != execution_id
                    or grant.get("practice_id") != "P04" or grant.get("activity_id") != "H04"
                    or grant.get("contract_version") != "p04-guardrail-v1"):
            ...
            execution = self.worker(source_path, grant["body"], grant["guardrail"], configuration, timeout=timeout)
