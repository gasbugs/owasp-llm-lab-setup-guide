"""Execute the P03 learner function in a bounded child; this is not a security sandbox."""
import asyncio
import contextlib
import hashlib
import json
...
def execute(source_path, current_job_id, configuration, *, timeout=75):
    ...
    try:
        ...
        if (not isinstance(current_job_id, str) or not current_job_id
                or len(current_job_id) > 128 or not current_job_id.isascii()
                or not all(c.isalnum() or c == "-" for c in current_job_id)):
            raise ValueError("invalid current job identity")
        payload = encode({"current_job_id": current_job_id, "configuration": configuration})
...
def child():
    ...
    try:
        ...
        namespace = {"__name__": "p03_learner"}
        with open(os.devnull, "w") as sink, contextlib.redirect_stdout(sink):
            exec(compile(envelope["source"], "learner.py", "exec"), namespace)
            function = namespace.get("search_current")
            if function is None:
                raise NotImplementedError()
            try:
                value = asyncio.run(function(envelope["input"]["current_job_id"], services))
            except ValueError:
                raise LearnerRejected() from None
        if (not isinstance(value, dict) or set(value) != {"status", "result"}
                or not ((value["status"] == "waiting" and value["result"] is None)
                        or (value["status"] == "ready" and isinstance(value["result"], dict)))):
