"""P03 persistent call lifecycle. This module makes no course verdict or AWS call."""
import hashlib
import hmac
import json
import math
...
CONTRACT = "p03-search-v1"
OPERATIONS = {"job_status", "retrieve"}


class LedgerError(RuntimeError):
    def __init__(self, status):
        super().__init__("P03 execution state is unavailable or incompatible")
        self.status = status
...
class SearchLedger:
    def __init__(self, path, *, now=time.time):
        ...
        with self.connect() as db:
            db.execute("""CREATE TABLE IF NOT EXISTS p03_executions (
                ...
                closed_at REAL)""")
            db.execute("""CREATE TABLE IF NOT EXISTS p03_calls (
                ...
                response_json TEXT, observation_id TEXT UNIQUE, provider_request_id TEXT UNIQUE,
    ...
    def register(self, suite_id, execution_id, source_digest, runner_digest, current_job_id):
        identifier(suite_id)
        identifier(execution_id)
        digest(source_digest)
        digest(runner_digest)
        if not isinstance(current_job_id, str) or not re.fullmatch(r"[A-Za-z0-9-]{1,128}", current_job_id):
            raise LedgerError(422)
        token, stamp = secrets.token_urlsafe(48), self.stamp()
        try:
            with self.connect() as db:
                db.execute("INSERT INTO p03_executions VALUES(?,?,?,?,?,?,?,NULL)",
                           (execution_id, suite_id, source_digest, runner_digest, current_job_id,
                            hashlib.sha256(token.encode()).hexdigest(), stamp))
        except sqlite3.IntegrityError:
            raise LedgerError(409) from None
        return {"practice_id": "P03", "activity_id": "H03", "contract_version": CONTRACT,
                "suite_id": suite_id, "execution_id": execution_id,
                "started_at": stamp, "current_job_id": current_job_id, "capability": token}

    def _execution(self, db, execution_id):
        row = db.execute("SELECT * FROM p03_executions WHERE execution_id=?", (execution_id,)).fetchone()
        if row is None:
    ...
    def reserve(self, token, suite_id, execution_id, operation, request_digest):
        ...
        with self.connect() as db:
            ...
            if row["closed_at"] is not None or not 0 <= stamp - row["started_at"] < 180:
                raise LedgerError(409)
            calls = db.execute("SELECT operation, state FROM p03_calls WHERE execution_id=?", (execution_id,)).fetchall()
            if any(call["state"] != "complete" or call["operation"] == operation for call in calls):
                raise LedgerError(409)
            db.execute("INSERT INTO p03_calls VALUES(?,?,?,?,?,?,NULL,NULL,NULL,NULL)",
    ...
    def finish(self, execution_id, operation, response=None):
        ...
        serialized, observation_id, provider_id = None, None, None
        ...
        try:
            with self.connect() as db:
                db.execute("BEGIN IMMEDIATE")
                row = self._execution(db, execution_id)
                call = db.execute("SELECT * FROM p03_calls WHERE execution_id=? AND operation=?",
                ...
                if row["closed_at"] is not None or call is None or call["state"] != "pending" or stamp < call["started_at"]:
                    raise LedgerError(409)
                db.execute("UPDATE p03_calls SET state=?,finished_at=?,response_json=?,observation_id=?,provider_request_id=? "
                           "WHERE execution_id=? AND operation=?",
                           ("complete" if response is not None else "error", stamp, serialized, observation_id, provider_id,
    ...
    def close(self, execution_id):
        with self.connect() as db:
            ...
            calls = db.execute("SELECT * FROM p03_calls WHERE execution_id=?", (execution_id,)).fetchall()
            stamp = self.stamp()
            if stamp < row["started_at"] or any(call["state"] == "pending" or stamp < call["finished_at"] for call in calls):
                raise LedgerError(409)
            db.execute("UPDATE p03_executions SET closed_at=? WHERE execution_id=?", (stamp, execution_id))
    ...
    def read(self, execution_id):
        with self.connect() as db:
            ...
            calls = [dict(call) for call in db.execute(
                "SELECT * FROM p03_calls WHERE execution_id=? ORDER BY sequence", (execution_id,))]
        row.pop("capability_hash")
        for call in calls:
            raw = call.pop("response_json")
            call["response"] = json.loads(raw) if raw is not None else None
        return {**row, "practice_id": "P03", "activity_id": "H03", "contract_version": CONTRACT,
