...
서버는 전용 EC2/SSM 터미널에서 Python으로 직접 실행합니다. `0.0.0.0`은 브라우저에서 여는 주소가 아니라, EC2의 모든 IPv4 인터페이스에서 연결을 받으라는 **bind sentinel**입니다. 다음 터미널은 서버가 실행되는 동안 그대로 열어 두고, 종료할 때 `Ctrl-C`를 누릅니다.
...
`TARGET_URL`은 계속 EC2 loopback HTTP origin만 허용합니다. `--host 0.0.0.0`은 미니 앱의 수신 bind만 바꾸며, 임베딩 backend `127.0.0.1:8012`를 외부에 노출하지 않습니다.

서버 터미널을 열어 둔 채 두 번째 EC2/SSM 터미널에서 health와 API를 `127.0.0.1`로 확인합니다. wildcard bind는 loopback 연결도 받으므로 이 주소가 올바른 로컬 검사 주소입니다.
...
미니 앱 process는 모든 IPv4 인터페이스에 bind합니다. Terraform Security Group은 TCP/18080을 `allowed_ingress_cidr`에 설정한 IPv4 `/32`에만 허용합니다. 기본값 `127.0.0.1/32`는 외부 직접 접속을 허용하지 않으므로 SSM forwarding을 사용합니다. 본인 공인 IPv4 `/32`로 설정해 Terraform을 적용했거나 이미 승인된 수동 all-protocol 규칙이 그 `/32`에 한정된 환경에서만 `http://EC2_PUBLIC_IP:18080` 직접 접속이 가능합니다. 권장 규칙은 TCP/18080 단일 포트이며 `0.0.0.0/0`으로 공개하면 안 됩니다. `http://0.0.0.0:18080`은 어느 경우에도 접속 URL로 사용하지 않습니다.
...
서버를 실행한 첫 번째 EC2/SSM 터미널에서 `Ctrl-C`를 누릅니다. Python 서버가 foreground에서 종료되고 해당 터미널의 prompt가 다시 표시되어야 합니다. 그 다음 두 번째 EC2/SSM 터미널에서 listener가 사라졌는지 확인합니다.
...
원본 예제는 commit 고정 setup repo에 남기고, 학습자가 수정한 `~/work/llm08-mini-app/learner_vector_app.py`는 필요에 따라 보관합니다. 로컬 port forwarding 터미널도 `Ctrl-C`로 닫은 뒤, [정본 가이드의 마지막 단계](../../docs/LLM08-SETUP.md#9-ec2는-모든-작업이-끝난-뒤-마지막에-중지)에 따라 EC2를 중지하고 `stopped`를 확인합니다.
