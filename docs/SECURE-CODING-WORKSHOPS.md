...
`tools/check_secure_coding_pairs.py`는 아홉 표식이 각각 한 번 존재하는지, 취약 호출이 활성화돼 있는지, 안전 호출이 바로 아래에 주석 상태로 있는지 검사한다. 관련 단위 테스트는 같은 API에서 전환 전후의 정책 결과가 실제로 달라지는지 검증한다. `tools/toggle_secure_coding_lab.py`는 E2E 게시자가 두 상태를 자동 빌드할 때만 사용하는 보조 도구이며 수강생 교재에는 노출하지 않는다. 수강생은 편집기에서 두 줄을 직접 바꿔 코드 차이를 읽는다.
