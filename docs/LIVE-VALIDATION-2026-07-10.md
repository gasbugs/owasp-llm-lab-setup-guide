...
- 최종 배포 판정: **NO-GO**
- 이유: setup `main` 기준 commit-addressed `sha-bc8e95d197c7382a2f9d83bf86325e119ad77328` 이미지가 registry에 없었다. 당시 GitHub Actions 정적 게이트는 통과했지만 외부 registry 로그인 단계가 자격 증명 부재로 실패했다.
...
1. 외부 credential이 필요 없는 내장 `GITHUB_TOKEN` publish와 공개 GHCR anonymous pull을 검증한다.
